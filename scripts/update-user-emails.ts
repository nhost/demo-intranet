...
async function updateUserEmails() {
	...
	p.log.info('This will allow you to impersonate any user by logging in with their modified email.');
	...
	try {
		...
		const UPDATE_USER_EMAIL_MUTATION = `
		...
		for (const user of users) {
			...
			const updateResponse = await nhost.graphql.post(
				{
					query: UPDATE_USER_EMAIL_MUTATION,
					variables: {
						userId: user.id,
						newEmail: newEmail,
					},
			...
			if (updateResponse.body.errors) {
				console.error(`❌ Failed to update ${user.email}:`, updateResponse.body.errors);
			} else {
		...
		console.log('The system will allow access with the modified email addresses.\n');
